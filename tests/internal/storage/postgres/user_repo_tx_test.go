...
func TestUserRepoTxCreate(t *testing.T) {
    ...
    mock.ExpectCommit()
    if err := txRepo.Create(ctx, u); err != nil { t.Fatalf("create tx: %v", err) }
    if err := uow.Commit(); err != nil { t.Fatalf("commit: %v", err) }
...
func TestUserRepoSoftDelete(t *testing.T) {
    ...
    if err := repo.Delete(context.Background(), id); err != nil { t.Fatalf("soft delete: %v", err) }
    if err := mock.ExpectationsWereMet(); err != nil { t.Fatalf("expect: %v", err) }
}
