...
func (s *cachedUserService) cacheKeyList(page, size int) string { return fmt.Sprintf("user:list:v%d:%d:%d", s.listVer.Load(), page, size) }
...
func (s *cachedUserService) Get(ctx context.Context, id uuid.UUID) (*User, error) {
    if u := s.getUser(ctx, id); u != nil { return u, nil }
...
func (s *cachedUserService) Delete(ctx context.Context, id uuid.UUID) error {
    if err := s.base.Delete(ctx, id); err != nil { return err }
    s.delUser(ctx, id)
    s.listVer.Add(1)
    return nil
}
...
func (s *cachedUserService) List(ctx context.Context, page, pageSize int) ([]*User, int, error) {
    ...
    if b, ok, _ := s.cache.Get(ctx, key); ok {
        var wrap struct { Users []*User `json:"u"`; Total int `json:"t"` }
    ...
    payload, _ := json.Marshal(struct { Users []*User `json:"u"`; Total int `json:"t"` }{users, total})
...
func (s *cachedUserService) getUser(ctx context.Context, id uuid.UUID) *User {
    ...
    if b, ok, _ := s.cache.Get(ctx, key); ok {
        var u User
        if json.Unmarshal(b, &u) == nil { return &u }
    }
    return nil
}
...
func (s *cachedUserService) delUser(ctx context.Context, id uuid.UUID) { s.cache.Delete(ctx, s.cacheKeyUser(id)) }
