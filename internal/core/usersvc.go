...
func normalizeEmail(e string) (string, error) {
    ...
    if e == "" { return "", fmt.Errorf("email empty: %w", ErrValidation) }
    if _, err := mail.ParseAddress(e); err != nil { return "", fmt.Errorf("invalid email: %w", ErrValidation) }
    return e, nil
}

func (s *userService) Create(ctx context.Context, name, email string) (*User, error) {
    if strings.TrimSpace(name) == "" { return nil, fmt.Errorf("name empty: %w", ErrValidation) }
...
func (s *userService) WithTx(ctx context.Context, fn func(context.Context, UnitOfWork) error) error {
    ...
    if !ok { return fmt.Errorf("transactions not supported") }
    uow, err := txStarter.BeginTx(ctx)
    if err != nil { return err }
    committed := false
    defer func() { if !committed { _ = uow.Rollback() } }()
    if err := fn(ctx, uow); err != nil { return err }
    if err := uow.Commit(); err != nil { return err }
    committed = true
    return nil
}
