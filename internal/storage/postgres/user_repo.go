...
func (r *txUserRepo) Create(ctx context.Context, u *core.User) error {
    ...
    if _, err := r.tx.ExecContext(ctx, q, u.ID, u.Name, u.Email, u.CreatedAt, u.UpdatedAt); err != nil { return fmt.Errorf("insert user: %w", err) }
    return nil
}
...
func (r *txUserRepo) List(ctx context.Context, page, pageSize int) ([]*core.User, int, error) {
    ...
    var total int
    if err := r.tx.QueryRowContext(ctx, `SELECT count(*) FROM users WHERE deleted_at IS NULL`).Scan(&total); err != nil { return nil, 0, err }
